...
void main(void) {
    ...
    if(centerCol.a != 0) {
        ...
        vec4 c1 = mix(freqs[3]-.3, 1., v) * vec4(1.5*freqs[2] * t * t* t , 1.2*freqs[1] * t * t, freqs[3]*t, 1.0)+c2+starcolor;

        gl_FragColor = vec4(c1.xyz, alpha.x);
    } else {
        for (float x = -radius; x <= radius; x++) {
            for (float y = -radius; y <= radius; y++) {
                ...
                if (currentColor.a != 0) {
                    ...
                    gl_FragColor = vec4(c1.xyz, alpha.y);
                }
            }
        }
    }
